...
function onSigint() {
    db.close(() => {
        console.log('MongoDB database connection was interrupted through app termination');
        process.exit();
    });
}
...
function connect() {
    ...
    // Generate a Test API Key to use for development automatically and remove it when we move over to production
    if ( process.env.NODE_ENV === 'development' ) {
        Token.findOne({ name: 'Test' }, (err, existing) => {
            ...
            const testToken = new Token({ name: 'Test', key: '123', dateCreated: new Date() });
